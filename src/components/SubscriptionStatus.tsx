...
import { calculateDaysRemaining, formatDaysRemaining, getSubscriptionStatus, getSubscriptionBadgeVariant } from "@/lib/utils";
...
export function SubscriptionStatus() {
  ...
  const isTrialPeriod = subscriptionData.subscription_tier === 'Trial' || 
                        (subscriptionData.trial_end && new Date(subscriptionData.trial_end).getTime() > Date.now());
  
  const daysRemaining = calculateDaysRemaining(
    isTrialPeriod ? subscriptionData.trial_end : subscriptionData.current_period_end
  );

...
export const formatCurrency = (value: number | string): string => {
  ...
  return new Intl.NumberFormat('pt-BR', NUMBER_FORMATS.CURRENCY).format(numValue);
};
...
export const formatPercentage = (value: number): string => {
  ...
  return new Intl.NumberFormat('pt-BR', NUMBER_FORMATS.PERCENTAGE).format(value / 100);
};

// Number formatting
export const formatNumber = (value: number, options?: Intl.NumberFormatOptions): string => {
  if (isNaN(value)) return '0';
  return new Intl.NumberFormat('pt-BR', options || NUMBER_FORMATS.DECIMAL).format(value);
};
...
export const applyPhoneMask = (value: string): string => {
  ...
  if (cleanValue.length <= 2) {
    ...
    const hasNinthDigit = cleanValue.length === 11;
    if (hasNinthDigit) {
...
export const calculateDaysRemaining = (endDate: string | Date | null): number => {
  if (!endDate) return 0;
...
export const formatDaysRemaining = (days: number, isTrialPeriod: boolean = false): string => {
  if (days <= 0) {
  ...
  return isTrialPeriod ? `${days} dias de trial restantes` : `${days} dias restantes`;
};
...
export const formatUserBadgeText = (userId: string, currentUserId: string, userName: string): string => {

...
export const useIAConfiguracoes = () => {
  ...
  const fetchConfiguracao = async () => {
    try {
      ...
      // Use the secure view that automatically decrypts API keys for the authenticated user
      const { data, error } = await supabase
        .from('ia_configuracoes_decrypted')
        .select('*')
        .eq('user_id', user.id)
        .maybeSingle();
  ...
  const salvarConfiguracao = async (apiKey: string, modelo: string) => {
    ...
    isLoading,
    salvarConfiguracao,
    isConfigured: !!configuracao?.api_key && configuracao?.api_key !== '[ENCRYPTED]'
  };
};
...
export const useSharedUsers = () => {
  ...
  const addSharedUser = async (userData: CreateSharedUserData) => {
    ...
    if (sharedUsers.length >= 3) {
      toast({
        ...
        description: "Você pode compartilhar sua conta com no máximo 3 usuários.",
        variant: "destructive",
      });
  ...
  const updateSharedUser = async (sharedUserId: string, updates: Partial<Pick<SharedUser, 'name' | 'whatsapp'>>) => {
    if (!user) return;
